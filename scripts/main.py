...
def generate_violating_traces(ideal_spec_path, mutated_specs):  # REWRITE
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        for i, spec in enumerate(mutated_specs):
            ...
            trace_file = os.path.join(temp_dir, f"trace_{i}.txt")
            try:
                # print("iideal:", ideal_spec_path)
                # print("mutated_spec_path:", mutated_spec_path)
...
def derive_initial_mode_dec(spec):  ##REWRITE
    constants = set()
    predicates = set()
    for line in spec:
    ...
    mode_dec = {
        ...
        "constant_c2": f"#constant(t2, {random.choice(list(constants))}).",
        "maxv": "#maxv(2).",
    }
    return mode_dec


def check_unique_mutations(new, mutated_specs):  ##REWRITE
    # unique = True
    # for i in range(len(mutated_specs)):
    #     for j in range(i + 1, len(mutated_specs)):
    #         if mutated_specs[i] == mutated_specs[j]:
    #             print(f"Mutation {i} and Mutation {j} are identical.")
    #             unique = False
